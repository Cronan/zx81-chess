...
def load_symbols(path='chess.sym'):
    """Parse the pasmo symbol table into {name: address}.

    Generated by `make build` alongside chess.bin. Lines look like:
        think		EQU 042ADH
    Tests resolve routine addresses from this instead of hard-coding
    offsets or scanning for byte patterns, so reordering chess.asm
    can't silently break the test locators.
    """
    symbols = {}
    with open(path) as f:
        for line in f:
            ...
            if len(parts) == 3 and parts[1] == 'EQU':
                symbols[parts[0]] = int(parts[2].rstrip('Hh'), 16)
    return symbols
...
def main():
    ...
    # Routine addresses come from the pasmo symbol table
    try:
        symbols = load_symbols()
    except FileNotFoundError:
        print("ERROR: chess.sym not found. Run: make build")
        sys.exit(1)

    start_addr = symbols['start']
    print(f"Entry point: ${start_addr:04X}")

    # --- TEST 1: Run init_board ---
    print("\n=== TEST 1: Board Initialisation ===")

    init_addr = symbols['init_board']
    ...
    draw_addr = symbols['cls_and_draw']
