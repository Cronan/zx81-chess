...
```asm
    ; 8 rows of the board
    ld      c, 8            ; Row counter
    ld      de, board + 56  ; Start at rank 8 (display top-down)
```

DE tracks our position in the board array, starting at board + 56 (rank 8, displayed at the top of the screen) and working backwards. An early version used IX for this, but every IX instruction carries a one-byte $DD prefix and `ADD IX, DE` doesn't exist, so plain DE - saved on the stack around the ROM print calls - is cheaper.
...
**Register pressure:** We need B for the column counter (DJNZ), and DE holds the board pointer. PUSH/POP only works with register pairs, so we save BC and DE on the stack around `call get_piece_char` (which trashes DE and HL) and the ROM print calls.
...
```asm
    ...
    ld      hl, -16         ; DE is one past this rank;
    add     hl, de          ; 16 back = start of the rank below
    ex      de, hl
...
After the column loop DE has advanced to one past the rank just printed, so stepping back 16 lands on the start of the rank below. `EX DE, HL` swaps the result back into DE in one byte.
...
**The INC A trick:** "no key" is $FF, and `CP $FF` costs two bytes - but `INC A` wraps $FF to zero, setting the Z flag for one byte, and a `DEC A` on the way out restores the real key code. With LAST_K's address held in HL, both the read and the clear are one-byte (HL) operations too.
...
dm_pawn:
    ...
    jr      nz, dm_rearm
    ld      a, b            ; XOR merge: source rank bits
    xor     c               ; with destination file bits
...
**The bypassed pawn's address:** it sits on the *source* rank (the capturing pawn moves diagonally off that rank) in the *destination* file - `(C AND $38) OR (B AND $07)`, one expression that works for both colours. The classic XOR bit-merge `B XOR ((B XOR C) AND $38)` computes it in 5 bytes: the inner XOR finds the bits where the squares differ, the AND keeps only the rank bits, and the outer XOR flips exactly those in B - replacing B's rank with C's while keeping B's file.
...
### gen_knight / gen_king: One Loop, Two Pieces

The Knight and the King look nothing alike on the board, but their move generators are *identical*: try 8 single-step offsets from a table, reject off-board and column-wrapped targets, reject own pieces, score the rest. The only differences are which table and how far the column may legally change. So there is only one loop:
...
gnk_loop:
    ...
    jr      nc, gnk_skip
    ; ...occupancy check, score_move, try_move...
```

The column-delta limit rides in the D register, which is free here (the piece type the think dispatch left in D isn't needed again) and is preserved by every helper the loop calls. Merging the two generators saved 36 bytes in one go - the single biggest saving in the deduplication pass.
...
### board_addr / lut / get_board_sq: The Indexing Helpers
...
"Index a table by the value in A" is the single most repeated idiom in the program - the board itself, the piece-value table, and the piece-character table are all indexed the same way. `lut` is the generic core (table in HL, index in A, clobbers DE); `board_addr` is just `LD HL, board` falling through into it; `get_board_sq` wraps that for callers inside register-sensitive loops (square in C, everything but A preserved). `score_move` and `get_piece_char` call `lut` directly with their own tables. Factoring this out of seven inlined copies paid for a good chunk of the en passant feature.
...
```
 "Every instruction was chosen.
  ...
  Total: 918 bytes
  Banked under the 984-byte ceiling: 66
```
